...
{
    public static $ROUTE_NAME ='admin.statistics.directions';
    ...
    {
        $keyboard = [];
        $directions = Direction::withCount('announcements')->orderBy('announcements_count')->offset(($page - 1) * $limit)->limit($limit)->get();

        foreach($directions as $direction){
            $keyboard[] =  [Keyboard::inlineButton(['text' => "#$direction->id - $direction->name: $direction->announcements_count ta", 'callback_data' =>  self::$ROUTE_NAME . ":show($$page)"])];
        }
        return $keyboard;
    }
