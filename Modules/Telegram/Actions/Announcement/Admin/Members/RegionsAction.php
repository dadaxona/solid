...
{
    public static $ROUTE_NAME ='admin.members.regions';
    ...
    {
        $keyboard = [];
        $regions = Region::withCount('telegram_users')->orderBy('telegram_users_count')->offset(($page - 1) * $limit)->limit($limit)->get();
        foreach($regions as $region){
            $keyboard[] =  [Keyboard::inlineButton(['text' => "#$region->id - $region->name: $region->telegram_users_count ta", 'callback_data' =>  self::$ROUTE_NAME . ":show($$page)"])];
        }
        return $keyboard;
    }
